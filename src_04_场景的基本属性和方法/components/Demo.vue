...
const ctrlObj = { rotationSpeed: 0.01, jumpSpeed: 0.01, foggy: 0.01 }
const ctrlObj2 = new function () {
    this.addNewCube = function () {
        const tempGeometry = new THREE.BoxGeometry(2, 2, 2);
        const tempMaterial = new THREE.MeshLambertMaterial({ color: 0xff0000 });
        const temp = new THREE.Mesh(tempGeometry, tempMaterial)
        temp.castShadow = true;
        temp.position.x = -Math.random() * 20;
        temp.position.y = Math.random() * 20;
        temp.position.y = Math.random() * 20;
        scene.add(temp);
    }
}
